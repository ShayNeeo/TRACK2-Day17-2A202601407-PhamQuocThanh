...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Sử dụng ON CONFLICT (event_id) DO UPDATE để đảm bảo tính idempotent.
    Bọc transaction để DuckDB ghi theo lô cực nhanh thay vì 500 transaction lẻ.
    """
    con.begin()
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # Kết hợp với ON CONFLICT DO UPDATE ở write_batch để đạt idempotency.
