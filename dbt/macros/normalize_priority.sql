...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or trim({{ col }}) = '' then 'Giá trị rỗng hoặc NULL'
        when try_cast({{ col }} as integer) is not null then 'Số nguyên ngoài miền 1..4'
        else 'Chuỗi định dạng không xác định'
    end
{% endmacro %}
